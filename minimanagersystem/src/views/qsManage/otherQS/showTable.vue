					...
					<el-row>
						<el-col :span="8">
							<el-form-item label="章节：" label-width="110px">
								<el-select v-model="query.chapter" filterable placeholder="请选择">
								    <el-option
						...
						<el-col :span="8">
							<el-form-item label="难度：" label-width="110px">
								<el-select v-model="query.difficulty" filterable placeholder="请选择">
		...
		<div class="Icontainer">
			<div class="IcontainerTopRow">
				...
				<div class="IcontainerTopBtns">
					...
					<el-button type="primary" @click="addNewQuestion()">{{ addButtonValue }}</el-button>
				</div>
			</div>
			<el-table fit border :data="this.$store.state.tableData" row-key="index" :height="tableHeight" stripe style="width: 100%"
...
<script>
	...
	export default {
		...
		mounted() {
			window.onresize = () => {
				this.$nextTick(() => {
					this.tableHeight = document.body.clientHeight - 330 + 'px';
				});
			};
			this.$nextTick(() => {
				this.tableHeight = document.body.clientHeight - 330 + 'px';
