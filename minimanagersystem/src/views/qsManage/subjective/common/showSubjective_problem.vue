    ...
    <el-row>
      <el-col class="card-box" :span="24" v-if="isshowCard">
        <div class="gradevalue">
          ...
          <el-slider v-model="data.grade" show-input> </el-slider>
        </div>
        <div>
        ...
        <el-card
          class="box-card"
          v-for="(item, index) in data.problemsAnswer"
          :key="index"
        >
          <div slot="header" class="clearfix">
            <span class="card-title">第{{ index + 1 }}题</span>
            <button class="card_btn" :data-id="index" @click="getFatherData">
              添加备注
            </button>
          </div>
        ...
        <div class="subBtn" v-if="isshowCard">
          <el-button type="primary" class="cardSubmit" @click="onSubmit(data)"
            >提交</el-button
          >
        </div>
      </el-col>
      <el-col class="card-box" :span="24" v-if="!isshowCard">
        <div>
          ...
          <Editor
            class="CardModifyDataTestArea"
            v-on:input="GetinputData"
            v-model="fatherData"
          ></Editor>
          <el-button type="primary" class="backCard" @click="BackCard"
            >提交</el-button
          >
        </div>
      </el-col>
...
export default {
  ...
  methods: {
    ...
    BackCard() {
      for (let i = 0; i < this.data.problemsAnswer.length; i++) {
        this.data.problemsAnswer[i] = this.data.problemsAnswer[i].replace(
          /\<.*?\>/g,
          ""
        );
      }
  ...
  beforedestroyed() {
    window.addEventListener("load", function() {
      ...
      if (this.$route.path === "/Managerindex/subjectiveItem") {
        // /date 表示日期选择路由
